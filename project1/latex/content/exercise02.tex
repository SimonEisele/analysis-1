In der zweiten Übung geht es um die Darstellung von Graphen einer Kurve in unterschiedlichen Skalen. Dazu plotten wir jeweils die Funktion
\begin{equation}
...
Dieser skaliert die X-Achse logarithmisch ($\log_{10}$), während er die Y-Achse linear belässt. Wie in Abbildung~\ref{fig:exercise02-log-linear-semilogx} ersichtlich,
erscheint die Kurve exponentiell (enstspricht dem Verhalten einer Exponentialfunktion).
...
skaliert beide Achsen logarithmisch ($\log_{10}$). Wie in Abbildung~\ref{fig:exercise02-log-log-loglog} ersichtlich, führt dies zu einer Geraden, da Potenzfunktionen
im log-log-Diagramm linear werden.
...
\begin{align*}
    v &= \log_{10}(f(x)) \\
      ...
      &= 2u + \log_{10}(3)
\end{align*}
\noindent Die Gerade hat also eine Steigung von $2$ und einen Y-Achsenabschnitt bei $y = \log_{10}(3)$.
