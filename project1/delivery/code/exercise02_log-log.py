...
# Calculate v-Values for linear plot (log10 transform)
vValues = np.log10(yValues)

###############################################################################
# Create plot log-log (semilogx)
plt.loglog(xValues, yValues, label=r'$f(x) = 3x^2$', linewidth=0.7)
plt.xlim(xValues.min(), xValues.max())
plt.xlabel('x')
plt.ylabel('f(x)')
plt.title('Darstellung eines Graphen (log-log) mit loglog()')
...
location = asksaveasfilename(
    ...
    initialfile="exercise02-log-log-loglog.png"
)
...
plt.plot(uValues, vValues, label=r'$f(x) = 3x^2$', linewidth=0.7)
plt.xlim(uValues.min(), uValues.max())
plt.xlabel(r'$u = log_{10}(x)$')
plt.ylabel(r'$v = log_{10}(f(x))$')
...
location = asksaveasfilename(
    ...
    filetypes=[("PNG files", "*.png")],
    title="Speicherort für Plot auswählen",
    initialfile="exercise02-log-log-plot.png"
)
